...
import { markTaskComplete, taskDueOverview } from '../actions/profileActions'
...
const Tasks = () => {
  ...
  // Access the correct state from reducer
  const { tasks: taskData, loading: tasksloading, error: taskerror } = useSelector((state) => state.taskDueOverviewReducer);
  console.log("Tasks State:", taskData, tasksloading, taskerror);
  
  const [filter, setFilter] = useState('all');
  const [searchTerm, setSearchTerm] = useState('');
  const [currentPage, setCurrentPage] = useState(1);

  // Load tasks based on current filters and search
  useEffect(() => {
    const filters = {
      filter: filter,
      page: currentPage,
    ...
    // Only add search query if there's actual search text
    if (searchTerm.trim() !== '') {
      filters.q = searchTerm;
    }

    dispatch(taskDueOverview(filters));
  }, [dispatch, filter, searchTerm, currentPage]);

  const handleMarkComplete = (taskId) => {
    if (taskId) {
      dispatch(markTaskComplete(taskId));
      // Reload tasks after completion with current filters
      setTimeout(() => {
        const filters = {
          filter: filter,
          page: currentPage,
  ...
  // Get tasks from response - FIXED: Access the correct data structure
  const taskList = taskData?.tasks || [];
  const totalTasks = taskData?.total_tasks || 0;
  const completedTasks = taskList.filter(task => task.is_completed).length;
  const overdueTasks = taskList.filter(task => isOverdue(task.due_date, task.is_completed)).length;

  // Pagination info from server
  const paginationInfo = taskData;

  console.log("Task Data:", taskData); // Debug log
  console.log("Task List:", taskList); // Debug log
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => dispatch(taskDueOverview({ filter, page: currentPage }))}
              ...
              <motion.button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                  paginationInfo.has_previous
                    ? 'bg-indigo-600 text-white shadow-lg cursor-pointer hover:bg-indigo-700'
            ...
            >
              ...
              <p className="text-gray-600 max-w-md mx-auto mb-6">
                {searchTerm 
                  ...
                  : "No tasks available for the current filters."}
              </p>
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                onClick={() => {
                  ...
                  setFilter('all');
                  setCurrentPage(1);
                  dispatch(taskDueOverview({ filter: 'all', page: 1 }));
                }}
                      ...
                      <div className="flex-1 min-w-0 space-y-3">
                        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            ...
                            {/* Stage Badge - Use stage_display from backend */}
                            {task.stage_display && (
                              <span
